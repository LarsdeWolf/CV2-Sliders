...
def plot_light_pos(input_img, threshold):
    ...
    if labels.max() == 0:
        ...
        for region in regionprops(labels):
            weighted_center_of_mass = region.weighted_centroid
            print("Light source detected in position: x:", int(weighted_center_of_mass[1]), ",y:",
                  int(weighted_center_of_mass[0]))
            light_positions.append((int(weighted_center_of_mass[1]), int(weighted_center_of_mass[0])))
...
class Flare_Image_Loader(data.Dataset):
    def __init__(self, image_path, flare_strengths, transform_base=None, transform_flare=None, mask_type=None):
        ...
        self.flare_strengths = flare_strengths
    ...
    def __getitem__(self, index):
        ...
        light_positions = plot_light_pos(base_img, 0.97 ** gamma)
        ...
        for light_pos in light_positions:
            ...
            transform_flare = transforms.Compose([transforms.RandomHorizontalFlip(),
                                                  ...
                                                  transforms.RandomAffine(degrees=(0, 360), scale=(0.8, 1.5),
                                                                          translate=(0, 0), shear=(-20, 20)),
            ...
            if self.reflective_flag and not applied:
            ...
            if final_image is None:
                final_image = flare_img
            else:
                final_image += flare_img

        images = {}
        for strength in self.flare_strengths:
            # merge image
            merge_img = final_image * strength + base_img
            merge_img = torch.clamp(merge_img, min=0, max=1)

            if self.mask_type is None:
                images[strength] = [adjust_gamma_reverse(base_img), adjust_gamma_reverse(flare_img),
                                    adjust_gamma_reverse(merge_img), gamma]
                continue
            ...
            images[strength] = [adjust_gamma_reverse(base_img), adjust_gamma_reverse(flare_img),
                                adjust_gamma_reverse(merge_img), flare_mask, gamma]
        return images
